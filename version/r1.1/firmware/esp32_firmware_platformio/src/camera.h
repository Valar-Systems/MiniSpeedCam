...
// --- OV2640 to ESP32-S3 pin mapping (board revision 1.1) ---
...
int cameraSetup(void) {
  camera_config_t config = {};  // zero-init so fields we don't set explicitly (e.g. sccb_i2c_port) start at 0, not garbage
  ...
  */
  ...
  config.fb_count = 2;  // double-buffer so Core 1 can capture a new frame while Core 0 still streams the previous one
